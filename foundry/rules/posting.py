...
class PostingRuleProcessor:
    ...
    def _apply_coa_enrichments(self, df: DataFrame) -> DataFrame:
        ...
        df = self._atlas.apply(df, mapping_name='DEPARTMENT_MAPPING')
        df = self._atlas.apply(df, mapping_name='AFFILIATE_CODE_MAPPING')
        df = self._atlas.apply(df, mapping_name='ACCOUNT_TB_MAPPING')
        df = self._atlas.apply(df, mapping_name='CR_DR_MAPPING')
