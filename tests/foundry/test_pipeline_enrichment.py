...
class _EnrichmentPipeline(BasePipeline):
    ...
    def __init__(self, enrichment_df, raise_error=False, **kwargs):
...
def _make_pipeline(enrichment_df, run_tracker, raise_error=False):
...
def test_enrichment_returns_zone_result_and_completes_execution(spark):
    ...
    pipeline = _make_pipeline(enrichment_df=df, run_tracker=run_tracker)

    workflow_run_id = uuid4()

    result = pipeline.enrichment(workflow_run_id=workflow_run_id)
...
def test_enrichment_passes_parent_run_id_through(spark):
    run_tracker = MagicMock(spec=RunTracker)
    execution_run = _make_execution_run()
    run_tracker.start_execution.return_value = execution_run
...
def test_enrichment_fails_execution_and_reraises_on_error(spark):
    run_tracker = MagicMock(spec=RunTracker)
    execution_run = _make_execution_run()
    run_tracker.start_execution.return_value = execution_run
